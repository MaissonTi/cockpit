...
export function usecaseOnlyRepoConstructor(plopType: PlopType): void {
  ...
  if(answers?.is_connection_repository){
    value = 'constructor(private readonly {{ camelCase selected_module }}Repository: I{{ pascalCase selected_module }}Repository) {}\n'
  }
  plop.setPartial('usecaseOnlyRepoConstructor', plop.renderString(value, answers));
}
