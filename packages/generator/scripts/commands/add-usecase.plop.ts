...
function addModuleUserCase({ plop, answers }: PlopType) {
  ...
  injectModule(
    ...
    modulePathUsecases,
    [EnumImport.Module],
    [ActionReplace.Imports, ActionReplace.Exports],
  );
}

function addModuleController({ plop, answers }: PlopType) {
  const modulePathController = path.resolve(
    ROOT_MONOREPO,
    'apps/api/src/presentation/_presentation.module.ts',
  );
  injectModule(
    { plop, answers },
    modulePathController,
    [EnumImport.Controller],
    [ActionReplace.Controllers],
  );
}
...
function addUseCaseInController({ plop, answers }: PlopType) {
  ...
  injectModule(
    ...
    [ActionReplace.ControllersAddConstructor, ActionReplace.ControllersAdd],
  );
}
...
function addUseCaseInModule({ plop, answers }: PlopType) {
  ...
  if (answers.is_connection_repository) {
    return injectModule(
      ...
      modulePathUsecases,
      [EnumImport.UseCase],
      [ActionReplace.ProvidersRepoFactory, ActionReplace.ExportsFactory],
    );
  }
  ...
  return injectModule(
    ...
    modulePathUsecases,
    [EnumImport.UseCase],
    [ActionReplace.ProvidersFactory, ActionReplace.ExportsFactory],
  );
}
