...
export async function validateActionZod(params: ParamsZod ): Promise<ResponseAction> {
  ...
  if (!result.success) {
    const errors = Object.fromEntries(
      Object.entries(result.error.flatten().fieldErrors).map(([key, value]) => [key, value ?? []])
    );
...
export function validateZod<T>(params: Omit<ParamsZod, 'service'> ): ResponseAction<T> {
  ...
  return { success: true, message: null, errors: null, data: result.data as z.infer<typeof schema> }
}
