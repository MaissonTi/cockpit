...
export function useTForm<T extends FieldValues = FieldValues>(
  ...
  const { register, handleSubmit: submit, reset, ...rest } = useForm<T>({
  ...
  const [dataForm, setDataForm] = useState<FormState<T>>(
    { data: initialState, success: false, message: null, errors: null },
  )
  ...
  return { dataForm, handleSubmit: submit(handleSubmit), register, reset, isPending, ...rest } as unknown as UseTFormReturn
}
